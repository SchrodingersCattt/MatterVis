...
def register_overlay_routes(v2, backend) -> dict:
    ...
    # ----- overlay_overrides -------------------------------------------
    #
    # Per-scene manual 2D component positions. Compass/legend style entries
    # use paper anchors; atom/bond/polyhedron labels use world anchors plus
    # pixel offsets so they can be reprojected after camera changes.

    @v2.get("/overlay_overrides")
    def overlay_overrides_list():
        return jsonify({
            "overrides": backend.list_overlay_overrides(scene_id=request.args.get("scene_id")),
        })

    @v2.post("/overlay_overrides")
    def overlay_overrides_create():
        payload = request.get_json(force=True, silent=True) or {}
        try:
            override = backend.add_overlay_override(
                payload,
                scene_id=_scene_id_from_request(),
                override_id=payload.get("id"),
            )
        except ValueError as exc:
            return jsonify({"error": str(exc)}), 400
        return jsonify(override)

    @v2.patch("/overlay_overrides/<override_id>")
    def overlay_overrides_update(override_id: str):
        payload = request.get_json(force=True, silent=True) or {}
        try:
            override = backend.update_overlay_override(
                override_id,
                patch=payload,
                scene_id=_scene_id_from_request(),
            )
    ...
    @v2.delete("/overlay_overrides/<override_id>")
    def overlay_overrides_delete(override_id: str):
        ok = backend.remove_overlay_override(
            override_id,
            scene_id=request.args.get("scene_id"),
        )
        if not ok:
            return jsonify({"error": f"unknown overlay override id: {override_id!r}"}), 404
        return jsonify({"deleted": override_id})

    @v2.post("/overlay_overrides/reorder")
    def overlay_overrides_reorder():
        payload = request.get_json(force=True, silent=True) or {}
        ordered = payload.get("order")
        if not isinstance(ordered, list):
            return jsonify({"error": "'order' must be a list of overlay override ids"}), 400
        try:
            overrides = backend.reorder_overlay_overrides(
                ordered_ids=ordered,
                scene_id=_scene_id_from_request(),
            )
        except ValueError as exc:
    ...
    # written by ``mat_viewer.perf_log``. It is the missing
