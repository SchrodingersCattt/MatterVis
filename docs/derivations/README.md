...
at the loader boundary with `M = legacy_M.T` in `mat_viewer/scene/core.py`.
...
scene in `mat_viewer/scene/core.py:503-505`.
...
`mat_viewer/compass/core.py:46-87`.
...
- `mat_viewer/scene/core.py` converts the static-publication parser output into row
  vectors.
- `mat_viewer/scene/core.py:156` passes `M.T` back into a legacy helper that
