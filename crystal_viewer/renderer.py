...
def _scene_ranges(scene: dict, style: dict, topology_data: dict | None = None):
    ...
    mins = atom_mins - pad
    maxs = atom_maxs + pad
    # Equalize all three axis ranges to a cube so Plotly never applies
    # anisotropic stretching via aspectmode="data".
    spans = maxs - mins
...
def _minor_bond_wireframe_traces(scene: dict, style: dict):
    ...
    for bond in scene["bonds"]:
        bond_occ = float(bond.get("occ", 1.0))
        if bond_occ >= 0.999 and not bond.get("is_minor"):
            continue
...
def _minor_outline_traces(scene: dict, style: dict):
    ...
    for color, minors in groups.items():
        for center, radius, occ in minors:
            # Thicker ring for lower occupancy
...
def axis_key_overlay(scene: dict, style: dict) -> tuple[list[dict], list[dict]]:
    ...
    if not style.get("show_axis_key", False):
        return [], []
    # Prefer camera-aware projections that account for Plotly's per-axis
    # aspect scaling (cube_scale).  Fall back to the cached build-time
    # projections only when the camera path is unavailable.
    projections = None
    try:
        from .render.viewport import _camera_axis_projections

        projections = _camera_axis_projections(scene, style)
    except Exception:
        pass
...
def build_row_figure(
    ...
    # When the same scene dict is reused across columns (e.g. different
    # disorder styles on the same structure), the mesh trace cache lives
    # on the scene dict and later columns would mutate traces belonging
    # to earlier ones.  Deep-copy duplicates so each column gets its own
    # independent cache.
    seen_ids: set[int] = set()
    deduplicated: list[tuple[dict, dict]] = []
    for scene, style in scene_style_pairs:
        sid = id(scene)
        if sid in seen_ids:
            scene = copy.deepcopy(scene)
        seen_ids.add(sid)
        deduplicated.append((scene, style))

    all_trace_dicts: list[dict] = []
    for col_idx, (scene, style) in enumerate(deduplicated):
        ...
        # Shallow-copy cached dicts so that mutating ``td["scene"]``
        # below does not poison the cache or earlier panels when the
        # same scene object is reused across multiple columns.
        trace_dicts.extend(dict(d) for d in mesh_payload["bond_dicts"])
        trace_dicts.extend(dict(d) for d in mesh_payload["minor_bond_dicts"])
        trace_dicts.extend(dict(d) for d in mesh_payload["atom_dicts"])
        trace_dicts.extend(dict(d) for d in mesh_payload["minor_outline_dicts"])
        ...
        # Merge into existing layout (preserves domain set by make_subplots)
        existing = layout_dict.get(scene_name, {})
        existing.update({
        ...
        layout_dict[scene_name] = existing
...
def build_figure(scene: dict, style: dict, topology_data: dict | None = None) -> go.Figure:
    ...
    # Inject camera into style so axis_key_overlay can compute
    # cube_scale-corrected projections matching the Plotly layout.
    compass_style = dict(style, camera=camera)
    key_annotations, key_shapes = axis_key_overlay(scene, compass_style)
    if key_annotations:
