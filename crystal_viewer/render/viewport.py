...
def _scene_ranges(scene: dict, style: dict, topology_data: dict | None = None):
    ...
    # Arbitrary mesh entities (for example an open through-cylinder) own the
    # viewport in exactly the same way as topology hulls: use their world
    # vertices, but reject a far-away optional overlay in unit-cell mode so it
    # cannot collapse the actual cell into a tiny strip.  The range calculation
    # deliberately does not project or approximate the mesh; the full vertex
    # envelope is what keeps a rotated/oblique entity from being clipped.
